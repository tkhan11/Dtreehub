...
if __name__ == '__main__':
	...
	for enableParallelism in parallelism_cases:
		...
		model = dtb.fit(df, config, validation_df = validation_df)
		...
		print(dtb.feature_importance(decision_rules))
		...
		dtb.evaluate(model, validation_df)

		dtb.save_model(model)
		print("built model is saved to model.pkl")

		restored_model = dtb.load_model("model.pkl")
		print("built model is restored from model.pkl")

		instance = ['Sunny', 'Hot', 'High', 'Weak']
		prediction = dtb.predict(restored_model, instance)
		...
		config = {'algorithm': 'ID3', 'enableParallelism': enableParallelism}
		model = dtb.fit(pd.read_csv("dataset/golf2.txt"), config)

		instance = ['Sunny', 85, 85, 'Weak']
		prediction = dtb.predict(model, instance)
		print("prediction for ", instance, "is ", prediction)
		...
		dtb.fit(pd.read_csv("dataset/golf2.txt"), config)
		...
		dtb.fit(pd.read_csv("dataset/golf.txt"), config)
		...
		dtb.fit(pd.read_csv("dataset/golf3.txt"), config)
		...
		config = {'algorithm': 'Regression', 'enableParallelism': enableParallelism}
		dtb.fit(pd.read_csv("dataset/golf4.txt"), config)
		...
		config = {'algorithm': 'ID3', 'enableParallelism': enableParallelism}
		model = dtb.fit(pd.read_csv("dataset/car.data"), config)
		...
		config = {'algorithm': 'C4.5', 'enableParallelism': enableParallelism}
		dtb.fit(pd.read_csv("dataset/car.data"), config)
		...
		df = dtb.feature_importance(decision_rules)
		print(df)
		...
		model = dtb.fit(pd.read_csv("dataset/car_reg.data"), config)
		...
		config = {'algorithm': 'C4.5', 'enableParallelism': enableParallelism}
		model = dtb.fit(pd.read_csv("dataset/none_train.txt"), config)
		test_set = pd.read_csv("dataset/none_test.txt")
		instance = test_set.iloc[3]
		print(instance.values, "->", dtb.predict(model, instance))
